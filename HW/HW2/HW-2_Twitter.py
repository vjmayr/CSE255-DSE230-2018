...
timer_start()
# YOUR CODE HERE
counts = userRdd.map(lambda x: (partition[x[0]] if x[0] in partition.keys() else 7, 1))                .reduceByKey(lambda x,y: x+y)                .sortByKey()
...
print_count(tokRdd)
tokRdd = tokRdd.sortBy(lambda x: x[1], ascending=False)

print_tokens(tokRdd.take(20))
...
grpTok = sorted(grpRdd.collect())
for i in range(8):
    print_tokens(grpTok[i][1][:10], i)
